...
namespace TestWH.Service.Implementation
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<Response<AuthenticationResponse>> AuthenticateAsync(AuthenticationRequest request, string ipAddress)
        {
            ...
            var res = new Response<AuthenticationResponse>();
            ...
            try
            {
                ...
                if (user == null)
                {
                    res.Errors.Add($"No Accounts Registered with {request.Email}.");
                    throw new Exception();
                ...
                if (!result.Succeeded)
                {
                    res.Errors.Add($"Invalid Credentials for '{request.Email}'.");
                    throw new Exception();
                }
                if (!user.EmailConfirmed)
                {
                    res.Errors.Add($"Account Not Confirmed for '{request.Email}'.");
                    throw new Exception();
        ...
        public async Task<Response<string>> RegisterAsync(RegisterRequest request, string origin)
        {
            ...
            {
                res.Errors.Add( $"Username '{request.UserName}' is already taken." );
            }
            if (res.Errors.Count()==0)
            {
                ...
                if (userWithSameEmail == null)
                {
                    ...
                    {
                        res.Errors.AddRange(result.Errors.Select(s => s.Description).ToList());
                        //throw new ApiException($"{result.Errors.ToList()[0].Description}");
                    }
                }
                else
                {
                    res.Errors.Add($"Email {request.Email } is already registered.");
                }
